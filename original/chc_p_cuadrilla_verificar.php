...
// ============================================================
// CARGAR CUADRILLA — Compatible PHP 5.6 (sin get_result, sin ??)
// ============================================================
$rutEscV   = mysqli_real_escape_string($conn, $rutPEC);
$resCuadV  = mysqli_query($conn, "
    ...
    WHERE cq.idcuadrilla = $idCuadrilla AND cq.rut_pec = '$rutEscV'
    LIMIT 1");
$cuadrilla = $resCuadV ? mysqli_fetch_assoc($resCuadV) : null;
...
$subtipos = array();
if($resSubV) { while($r=mysqli_fetch_assoc($resSubV)) $subtipos[]=$r; }

// Fechas — tabla real: planclases (no planclases_test)
$resFechasV = mysqli_query($conn,
    "SELECT cf.*,
...
$fechas = array();
if($resFechasV) { while($r=mysqli_fetch_assoc($resFechasV)) $fechas[]=$r; }

// Capacitaciones
$resCapsV = mysqli_query($conn,
    "SELECT * FROM chc_p_cuadrilla_capacitacion WHERE idcuadrilla=$idCuadrilla ORDER BY orden ASC");
$caps = array();
if($resCapsV) { while($r=mysqli_fetch_assoc($resCapsV)) $caps[]=$r; }
$capsMap = array();
foreach($caps as $c) $capsMap[$c['orden']] = $c;

// Debriefing
$resDebV = mysqli_query($conn,
    "SELECT * FROM chc_p_cuadrilla_debriefing WHERE idcuadrilla=$idCuadrilla LIMIT 1");
$debrief = $resDebV ? mysqli_fetch_assoc($resDebV) : null;
