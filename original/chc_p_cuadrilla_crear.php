...
// Compatible PHP 5.6: mysqli_query + real_escape en vez de get_result
// ============================================================
$sqlSolicitud = "
    SELECT 
    ...
    LIMIT 1
";
$resSol   = mysqli_query($conn, $sqlSolicitud);
$solicitud = $resSol ? mysqli_fetch_assoc($resSol) : null;
...
// IMPORTANTE: tabla real es 'planclases' (no planclases_test)
// ============================================================
$sqlActividades = "
    SELECT
        p.idplanclases,
...
if($idCuadrilla > 0) {
    // Cabecera
    $resCuad   = mysqli_query($conn, "SELECT * FROM chc_p_cuadrilla WHERE idcuadrilla = $idCuadrilla AND idsolicitud = $idSolicitud LIMIT 1");
    $cuadrilla = $resCuad ? mysqli_fetch_assoc($resCuad) : null;
    ...
    if($cuadrilla) {
        ...
        // Debriefing guardado
        $resDeb          = mysqli_query($conn, "SELECT * FROM chc_p_cuadrilla_debriefing WHERE idcuadrilla = $idCuadrilla LIMIT 1");
        $debriefGuardado = $resDeb ? mysqli_fetch_assoc($resDeb) : null;
    }
}
