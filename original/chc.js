...
// ===========================================
// ===========================================
// SECCIÓN 10: CUADRILLA CHC
// Agregar estas funciones al final del archivo chc.js
// ===========================================

/**
 * Navega al formulario de creación/continuación de cuadrilla
 * @param {number} idsolicitud - ID de la solicitud CHC
 * @param {number} idcuadrilla - 0 si es nueva, >0 si continúa una existente
 */
function irACuadrilla(idsolicitud, idcuadrilla) {
    var chcList = document.getElementById('chc-list');
    if(!chcList) {
        console.error('❌ No se encontró el contenedor chc-list');
        return;
    }

    // Mostrar loading mientras carga
    chcList.innerHTML = '<div class="text-center p-5">' +
        '<div class="spinner-border text-primary" role="status"></div>' +
        '<p class="mt-3">Cargando formulario de cuadrilla...</p>' +
        '</div>';
    ...
    fetch(url)
        ...
        .catch(function(error) {
            ...
            chcList.innerHTML = '<div class="alert alert-danger">' +
                '<i class="bi bi-exclamation-triangle me-2"></i>' +
                'Error al cargar el formulario de cuadrilla. Por favor recargue la página.' +
                '</div>';
        });
}

/**
 * Navega a la vista de verificación de cuadrilla
 * @param {number} idcuadrilla - ID de la cuadrilla
 */
function irAVerificarCuadrilla(idcuadrilla) {
    ...
    chcList.innerHTML = '<div class="text-center p-5">' +
        ...
        '<p class="mt-3">Cargando verificación...</p>' +
        '</div>';
    ...
    fetch('chc_p_cuadrilla_verificar.php?cuadrilla=' + idcuadrilla)
        ...
        .catch(function(error) {
            console.error('❌ Error al cargar verificación:', error);
            chcList.innerHTML = '<div class="alert alert-danger">' +
                '<i class="bi bi-exclamation-triangle me-2"></i>' +
                'Error al cargar la verificación. Por favor recargue la página.' +
                '</div>';
        });
}

/**
 * Ejecuta los scripts contenidos en HTML inyectado dinámicamente.
 * Necesario porque innerHTML no ejecuta <script> automáticamente.
 * @param {HTMLElement} contenedor - El elemento donde se inyectó el HTML
 */
function ejecutarScriptsHTML(contenedor) {
    var scripts = contenedor.querySelectorAll('script');
    scripts.forEach(function(scriptOriginal) {
        ...
        if(scriptOriginal.innerHTML) {
            nuevoScript.innerHTML = scriptOriginal.innerHTML;
        }
