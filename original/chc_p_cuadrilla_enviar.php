...
$sqlCuad = "
    ...
    LIMIT 1";
$rutEscEnv = mysqli_real_escape_string($conn, $rutPEC);
$sqlCuad   = str_replace(
    array('WHERE cq.idcuadrilla = ? AND cq.rut_pec = ?'),
    array("WHERE cq.idcuadrilla = $idcuadrilla AND cq.rut_pec = '$rutEscEnv'"),
    $sqlCuad
);
$resEnvC   = mysqli_query($conn, $sqlCuad);
$cuadrilla = $resEnvC ? mysqli_fetch_assoc($resEnvC) : null;
...
$resVerE = mysqli_query($conn, "SELECT COUNT(*) AS total,
                  SUM(CASE WHEN hora_inicio IS NOT NULL AND hora_termino IS NOT NULL THEN 1 ELSE 0 END) AS completas
           FROM chc_p_cuadrilla_fecha WHERE idcuadrilla = $idcuadrilla");
$ver = $resVerE ? mysqli_fetch_assoc($resVerE) : array('total'=>0,'completas'=>0);
